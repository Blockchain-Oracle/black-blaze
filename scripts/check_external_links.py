    ...
    try:
        ...
        if exc.code == 404 and url in EXPECTED_PRIVATE_URLS:
            return {
                ...
                "final_url": exc.url,
                "ok": True,
                "note": "expected unauthenticated response for private repository",
            }
        if exc.code in REACHABLE_AUTH_STATUSES:
            return {
                ...
                "final_url": exc.url,
                "ok": True,
                "note": "reachable but access/method/rate restricted",
            }
        if exc.code in {400, 404, 410, 500, 501}:
            ...
            try:
                ...
                return {
                    ...
                    "ok": get_exc.code in REACHABLE_AUTH_STATUSES,
                    "error": str(get_exc),
                }
        ...
        # A second method also distinguishes transient HEAD/DNS/TLS failures
        # from a consistently unreachable source without masking real errors.
        ...
        try:
            with urllib.request.urlopen(retry_request, timeout=timeout) as response:
                return {
                    ...
                    "final_url": response.url,
                    "ok": True,
                    "note": f"HEAD failed once ({type(exc).__name__}); GET retry succeeded",
                }
        except urllib.error.HTTPError as retry_exc:
            return {
                "url": url,
                "status": retry_exc.code,
                "final_url": retry_exc.url,
                "ok": retry_exc.code in REACHABLE_AUTH_STATUSES,
                "error": f"HEAD: {exc}; GET: {retry_exc}",
            }
            ...
            return {
                ...
                "status": None,
                "ok": False,
                "error": f"HEAD: {exc}; GET retry: {retry_exc}",
            }
...
def main() -> int:
    ...
    print(
        f"Checked {len(results)} URLs: {len(results) - len(failures)} reachable, {len(failures)} failed"
    )
    for item in failures:
        print(
            f"FAIL {item.get('status')} {item['url']} — {item.get('error', 'unknown error')}"
        )
